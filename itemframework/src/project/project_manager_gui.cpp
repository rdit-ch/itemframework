...
void ProjectManagerGui::setMainGuiElements()
{
    ...
    QAction* actionWorkspaceManager = GuiManager::instance()->action("actionWorkspaceManager");
...
void ProjectManagerGui::updateMenuEntries()
{
    ...
    QAction* actionWorkspaces = GuiManager::instance()->action("actionWorkspace");
    ...
    for (const QSharedPointer<AbstractWorkspace> workspace : _projectManager->recentWorkspaces()) {
        ...
        connect(action, &QAction::triggered, [this, workspace](){
            if (!openWorkspace(workspace)) {
                qWarning() << QString("Switch to workspace %1 failed.").arg(workspace->name());
            }
        });
...
bool ProjectManagerGui::openWorkspace(QSharedPointer<AbstractWorkspace> workspace)
{
    ...
    if(workspace->compare(_projectManager->currentWorkspace())){
